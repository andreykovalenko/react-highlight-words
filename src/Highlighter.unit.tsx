...
describe("Highlighter", () => {
  ...
  function getHighlighterChildren({
    ...
    return render(
      <Highlighter
        ...
        {...rest}
      />
    ).baseElement.children[0].children[0];
  }

  let consoleError: {
    (...data: unknown[]): void;
    apply: (ctx: Console, message: string) => void;
  };
  ...
  it("should properly handle empty searchText", () => {
    ...
    emptyValues.forEach((emptyValue) => {
      ...
      expect(node.querySelectorAll(HIGHLIGHT_QUERY_SELECTOR).length).to.equal(
        0
      );
  ...
  it("should support stateless functional components via :highlightTag", () => {
    ...
    // @ts-expect-error ts(7031)np0
    const HighlightTag = ({ highlightIndex, ...rest }) => <span {...rest} />;

    const node = getHighlighterChildren({
      autoEscape: true,
      highlightTag: HighlightTag as FunctionComponent,
      searchWords: ["text"],
      textToHighlight: "This is text",
    });
    const matches = node.querySelectorAll(HIGHLIGHT_QUERY_SELECTOR);
  ...
  it("should support custom findChunks prop function", () => {
    ...
    console.log("node2", node2.innerHTML);
  ...
  it("should render chucks with the appropriate classes when case-insensitive", () => {
    ...
    expect(allMatches[0].classList).toContain("this");
    expect(allMatches[1].classList).toContain("is");
    expect(allMatches[2].classList).toContain("text");
  });
  ...
  it("should render chucks with the appropriate classes when case-sensitive", () => {
    ...
    const allMatches = node.querySelectorAll("mark");
    expect(allMatches).to.have.length(3);
    expect(allMatches[0].classList).not.toContain("this");
    expect(allMatches[1].classList).toContain("is");
    expect(allMatches[2].classList).not.toContain("text");
  });
  ...
  it("should support stateless functional components via :unhighlightTag", () => {
    ...
    const UnHighlightTag = ({ highlightIndex, ...rest }) => <a {...rest} />;

    const node = getHighlighterChildren({
      autoEscape: true,
      unhighlightTag: UnHighlightTag as FunctionComponent,
      searchWords: ["text"],
      textToHighlight: "This is text",
    });
